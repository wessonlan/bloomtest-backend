...
public class ApiModuleServiceImpl implements ApiModuleService {
    ...
    public List<Tree<String>> getNodeTreeByProjectId(Long projectId) {
        ...
        List<ApiModule> apiModules = apiModuleMapper.selectList(wrapperApiModule.eq("projectId", projectId));
        // 配置
...
//        treeNodeConfig.setDeep(5);
    ...
    private void getDefaultNode(Long projectId) {
        ...
        if (apiModuleMapper.selectCount(wrapperApiModule) == 0) {
    ...
    public Long addNode(ApiModule node) {
        ...
        apiModuleMapper.insert(node);
        return node.getId();
    }

    public void renameNode(EditNodeRequest request) {
        // 根据传入的id查询出数据
        ApiModule apiModule = apiModuleMapper.selectById(request.getId());
        // 更新对象属性值，保存
        apiModule.setId(request.getId());
        apiModule.setName(request.getName());
        apiModule.setUpdateTime(new Date());
        apiModuleMapper.updateById(apiModule);
    }
    ...
    public int deleteNode(Long id) {
        ...
        wrapper.eq("id", id)
               .or()
               .eq("parentId", id);
        return apiModuleMapper.delete(wrapper);
    }
